...
OS_TASK(APP_taskZncb)
{
    ...
    uint8 au8StatusBuffer[MAX_PACKET_SIZE];
    ...
    if (OS_E_OK == OS_eCollectMessage(APP_msgZpsEvents, &sStackEvent)) {
     ...
     /* Not pushing the sequence number */
                    ...
                    case 0x8006:
                    {
                        ...
                        u16Length+=sApsZdpEvent.uZdpData.sMatchDescRsp.u8MatchLength;
                    ...
                    case 0x8011:
					{
						u16Length = sizeof(uint8);
						pu8Buffer += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUserDescRsp.u8Status,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUserDescRsp.u16NwkAddrOfInterest,sizeof(uint16) );
						pu8Buffer += sizeof(uint16);
						u16Length += sizeof(uint16);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUserDescRsp.u8Length,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer, &sApsZdpEvent.uZdpData.sUserDescRsp.szUserDescriptor, sApsZdpEvent.uZdpData.sUserDescRsp.u8Length);
						u16Length+=sApsZdpEvent.uZdpData.sUserDescRsp.u8Length;

						vSL_WriteMessage(E_SL_MSG_USER_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer);
					}
					break;

					/* User Descriptor Set Confirm */
                    case 0x8014:
					{
						u16Length = sizeof(uint8);
						pu8Buffer += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUserDescConf.u8Status,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sUserDescConf.u16NwkAddrOfInterest,sizeof(uint16) );
						u16Length += sizeof(uint16);

						vSL_WriteMessage(E_SL_MSG_USER_DESCRIPTOR_SET_CONFIRM, u16Length, au8StatusBuffer);
					}
					break;
					...
					case 0x8010:
					{
						u16Length = sizeof(uint8);
						pu8Buffer += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sComplexDescRsp.u8Status,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sComplexDescRsp.u16NwkAddrOfInterest,sizeof(uint16) );
						pu8Buffer += sizeof(uint16);
						u16Length += sizeof(uint16);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sComplexDescRsp.u8Length,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sComplexDescRsp.sComplexDescriptor.u8XMLTag,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sComplexDescRsp.sComplexDescriptor.u8FieldCount,sizeof(uint8) );
						pu8Buffer += sizeof(uint8);
						u16Length += sizeof(uint8);
						memcpy(pu8Buffer, &sApsZdpEvent.uZdpData.sComplexDescRsp.sComplexDescriptor.pu8CdData, sApsZdpEvent.uZdpData.sComplexDescRsp.sComplexDescriptor.u8FieldCount);
						u16Length += sApsZdpEvent.uZdpData.sComplexDescRsp.sComplexDescriptor.u8FieldCount;

						vSL_WriteMessage(E_SL_MSG_COMPLEX_DESCRIPTOR_RESPONSE, u16Length, au8StatusBuffer);
					}
					break;
                    ...
                    case 0x8005:
                    {
                         ...
                         pu8Buffer += sizeof(uint8);
                         int i;
                         for (i = 0;i<sApsZdpEvent.uZdpData.sActiveEpRsp.u8ActiveEpCount;i++)
                         {
                             memcpy(pu8Buffer,&sApsZdpEvent.uLists.au8Data[i],sizeof(uint8) );
                             u16Length += sizeof(uint8);
                             pu8Buffer += sizeof(uint8);
                         }
                         vSL_WriteMessage(E_SL_MSG_ACTIVE_ENDPOINT_RESPONSE, u16Length, au8StatusBuffer);
                         //memcpy(pu8Buffer,&sApsZdpEvent.uZdpData.sActiveEpRsp.pu8ActiveEpList,sApsZdpEvent.uZdpData.sActiveEpRsp.u8ActiveEpCount );
                    }
                    break;
			...
			case ZPS_EVENT_NWK_ROUTE_DISCOVERY_CONFIRM:
				...
				vLog_Printf(TRACE_APP,LOG_DEBUG, "\nDISC Confirm: Status=%d, NWK Status=%d\n",
												...
												sStackEvent.uEvent.sNwkRouteDiscoveryConfirmEvent.u8NwkStatus);

				memcpy(pu8Buffer, &sStackEvent.uEvent.sNwkRouteDiscoveryConfirmEvent.u8Status,sizeof(uint8));
				u16Length += sizeof(uint8);
				pu8Buffer += sizeof(uint8);
				memcpy(pu8Buffer, &sStackEvent.uEvent.sNwkRouteDiscoveryConfirmEvent.u8NwkStatus,sizeof(uint8));
				u16Length += sizeof(uint8);
				vSL_WriteMessage(E_SL_MSG_ROUTE_DISCOVERY_CONFIRM, u16Length, au8StatusBuffer);
			break;
